...
import cv2
from AI_trainer_unknown import preprocess_image

# Load the model
all_models = list_files('OCR/Training/models')
#model = tf.keras.models.load_model('OCR/Training/models/' + all_models[-1])
model_list = ["OCR/Training/models/" + all_models[-i] for i in range(1, 4)] 
...
def predict_car(img, model):
    ...
    return car_presence, certainty, prediction



def get_parking_space_picture(db_file, file_path, model):
...
""", (area_name,))
    ...
    for row in result:
        ...
        car_presence, current_certainty, prediction = predict_car(cropped_img, model)
    ...
    return available_spots_count, total_spots, certainty_list, image_list, car_presence_list, prediction
...
for j in range(len(model_list)):
    model = tf.keras.models.load_model(model_list[j])
    available_spots_count, total_spots, certainty_list, image_list, car_presence_list, prediction = get_parking_space_picture(database_full_path, image_path, model)
    if image_list == []:
